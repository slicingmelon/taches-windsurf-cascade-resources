<overview>
Skills use pure XML structure for consistent parsing, efficient token usage, and improved Cascade performance. This reference defines the required and conditional XML tags for skill authoring, along with intelligence rules for tag selection.
</overview>
...
**Purpose**: Immediate, actionable guidance. Gets Cascade started quickly without reading advanced sections.
...
<tag name="context">
**When to use**: Background or situational information that Cascade needs before starting.
...
**When to use**: Common mistakes that Cascade should avoid.
...
2. Observe where Cascade struggles or succeeds
...
Markdown headings are just visual formatting. Cascade must infer meaning from heading text.
</parsing_accuracy>
...
Reduce parsing errors. Cascade knows exactly where sections begin and end.
</benefit>
