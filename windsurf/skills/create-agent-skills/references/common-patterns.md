...
<flexible_guidance>
Use when Cascade should adapt the format based on context:
...
<principle>
Choose one term and use it throughout the skill. Inconsistent terminology confuses Cascade and reduces execution quality.
</principle>
...
Cascade must now interpret: Are "API routes" and "URLs" the same? Are "fields", "boxes", "elements", and "controls" the same?
</bad_example>
...
Cascade must now research and compare all options before starting. This wastes tokens and time.
</bad_example>
...
**Why it matters**: Vague descriptions prevent Cascade from discovering and using the skill appropriately.
</pitfall>
...
**Why it matters**: Cascade may only partially read deeply nested files. Keep references one level deep from SKILL.md.
</pitfall>
...
<description>
Keep SKILL.md concise by linking to detailed reference files. Cascade loads reference files only when needed.
</description>
...
- Cascade only reads relevant reference files
...
- Cascade can fix issues without guessing
...
<checklist_pattern>
<description>
For complex multi-step workflows, provide a checklist Cascade can copy and track progress.
</description>
