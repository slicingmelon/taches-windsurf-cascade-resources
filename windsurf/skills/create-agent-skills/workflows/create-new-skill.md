...
# If complex:
mkdir -p ~/.codeium/windsurf/skills/{skill-name}/workflows
mkdir -p ~/.codeium/windsurf/skills/{skill-name}/references
# If needed:
mkdir -p ~/.codeium/windsurf/skills/{skill-name}/templates  # for output structures
mkdir -p ~/.codeium/windsurf/skills/{skill-name}/scripts    # for reusable code
```
...
```bash
cat > ~/.codeium/windsurf/global_workflows/{skill-name}.md << 'EOF'
---
