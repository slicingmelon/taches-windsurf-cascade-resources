---
name: create-plans
description: Create hierarchical project plans optimized for solo agentic development. Use when planning projects, phases, or tasks that Cascade will execute. Produces Cascade-executable plans with verification criteria, not enterprise documentation. Handles briefs, roadmaps, phase plans, and context handoffs.
---

<essential_principles>

<principle name="solo_developer_plus_cascade">
You are planning for ONE person (the user) and ONE implementer (Cascade).
No teams. No stakeholders. No ceremonies. No coordination overhead.
The user is the visionary/product owner. Cascade is the builder.
</principle>
...
**Critical insight:** Cascade doesn't degrade at 80% - it degrades at ~40-50% when it sees context mounting and enters "completion mode." By 80%, quality has already crashed.
...
**Cascade automates everything that has a CLI or API.** Checkpoints are for verification and decisions, not manual work.

**Checkpoint types:**
- `checkpoint:human-verify` - Human confirms Cascade's automated work (visual checks, UI verification)
...
**Critical rule:** If Cascade CAN do it via CLI/API/tool, Cascade MUST do it. Never ask human to:
...
**Protocol:** Cascade automates work → reaches checkpoint:human-verify → presents what was done → waits for confirmation → resumes
...
<domain_expertise>
**Domain expertise lives in `~/.codeium/windsurf/skills/expertise/` (global) or `.windsurf/skills/expertise/` (project-local)**
...
| "Agent SDK", "agentic SDK", "agentic app" | expertise/with-agent-sdk |
...
```bash
cat ~/.codeium/windsurf/skills/expertise/[domain]/SKILL.md 2>/dev/null
# Or for project-local:
cat .windsurf/skills/expertise/[domain]/SKILL.md 2>/dev/null
```
...
cat ~/.codeium/windsurf/skills/expertise/macos-apps/references/core-data.md
cat ~/.codeium/windsurf/skills/expertise/macos-apps/references/swift-conventions.md
```
...
FINDINGS.md       → Research output (if research done)
    ↓
PLAN.md           → THE PROMPT (Cascade executes this)
    ↓
