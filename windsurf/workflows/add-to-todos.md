...
- Current timestamp: ask Cascade to get the current date and time
...
3. Extract todo content:
   - **With user-provided topic**: Use it as the focus/title for the todo and context heading
   - **Without a topic**: Analyze recent conversation to extract:
...
4. Append new section to bottom of file:
   ...
   - Make each section self-contained for future Cascade context handoff and continuation
...
- **Add structured format to add-to-todos** - Standardize todo entries with Problem/Files/Solution pattern. **Problem:** Current todos lack consistent structure, making it hard for Cascade to have enough context when revisiting tasks later. **Files:** `workflows/add-to-todos.md:22-29`. **Solution:** Use inline bold labels with required Problem and Files fields, optional Solution field.

- **Create check-todos workflow** - Build companion workflow to list and select todos. **Problem:** Need workflow to review outstanding todos and load context for selected item. **Files:** `workflows/check-todos.md` (new), `TO-DOS.md` (reads from). **Solution:** Parse markdown list, display numbered list, accept selection to load full context and remove item.
```
