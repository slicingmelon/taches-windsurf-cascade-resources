# Context Handoff for Windsurf Cascade
...
```bash
cp whats-next.md ~/.codeium/windsurf/global_workflows/
```
...
```
You: /whats-next
Cascade: [Analyzes conversation, writes whats-next.md]
...
Cascade: [Reads handoff document, understands context, resumes work]
```
...
Cascade: [Works on task, creates components, sets up data fetching...]
You: /whats-next
